...
create table Nacionalidades(
	codigo_nacionalidad_NAC int auto_increment not null primary key,
    abreviatura_NAC varchar (2),
    descripcion_NAC varchar (50) not null
);

insert into Nacionalidades(codigo_nacionalidad_NAC, abreviatura_NAC, descripcion_NAC) values
(1, 'AF', 'Afganistán'),
(2, 'AX', 'Islas Gland'),
(3, 'AL', 'Albania'),
(4, 'DE', 'Alemania'),
...
create table Generos(
	codigo_genero_GEN int not null auto_increment primary key,
    descripcion_GEN varchar(20)
);

insert into Generos(codigo_genero_GEN, descripcion_GEN) values
(1, 'Femenino'), 
(2, 'Masculino'), 
(3, 'No binario'), 
(4, 'Transgénero'), 
...
create table Provincias(
    codigo_provincia_PRO int not null auto_increment primary key,
    descripcion_PRO varchar(50)
);

insert into Provincias (codigo_provincia_PRO, descripcion_PRO) values
(1, 'Buenos Aires'),
(2, 'Buenos Aires-GBA'),
(3, 'Capital Federal'),
(4, 'Catamarca'),
...
create table Localidades(
    codigo_localidad_LOC int not null auto_increment,
    codigo_provincia_LOC int not null,
    descripcion_LOC varchar(50) not null,
    primary key(codigo_localidad_LOC, codigo_provincia_LOC),
    foreign key (codigo_provincia_LOC) references Provincias(codigo_provincia_PRO)
);

insert into Localidades (codigo_localidad_LOC, codigo_provincia_LOC, descripcion_LOC) values
(1, 1, '25 de Mayo'),
(2, 1, '3 de febrero'),
(3, 1, 'A. Alsina'),
...
create table TiposCuenta(
    ...
    descripcion_TCU varchar(20) not null unique
);

insert into TiposCuenta (codigo_tipo_cuenta_TCU, descripcion_TCU) values
(1, 'Caja de ahorro'),
(2, 'Cuenta corriente');

create table Cuentas(
	numero_cuenta_CU int not null primary key,
    cbu_CU int not null unique,
    fecha_creacion_CU date not null,
    saldo_CU decimal not null,
    estado_CU bit not null default 1,
    codigo_tipo_cuenta_CU int not null,
    dni_cliente_CU int not null,
    foreign key (codigo_tipo_cuenta_CU) references TiposCuenta(codigo_tipo_cuenta_TCU),
	foreign key (dni_cliente_CU) references Clientes(dni_CLI)
);

create table TiposMovimiento(
	codigo_tipo_movimiento_TMOV int primary key not null,
    descripcion_TMOV varchar(20) not null unique
);

insert into TiposMovimiento(codigo_tipo_movimiento_TMOV, descripcion_TMOV) values 
(1, 'Alta de cuenta'), 
...
create table Movimientos(
	codigo_movimiento_MOV int auto_increment primary key,
    fecha_MOV date not null,
    detalle_MOV varchar(30) not null,
    importe_MOV decimal not null,
    numero_cuenta_MOV int not null,
    codigo_tipo_movimiento_MOV int not null,
    foreign key (numero_cuenta_MOV) references Cuentas(numero_cuenta_CU),
    foreign key (codigo_tipo_movimiento_MOV) references TiposMovimiento(codigo_tipo_movimiento_TMOV)
);



create table Prestamos(
	codigo_prestamo_PRS int primary key auto_increment,
    fecha_PRS date not null,
    importa_pagar_PRS decimal not null,
    importe_pedido_PRS decimal not null,
    plazo_meses_PRS int not null,
    intereses_PRS decimal not null,
    importe_cuota_PRS decimal not null,
    otorgado_PRS bool not null,
    motivo_rechazo_PRS varchar(500),
    estado_PRS bit not null,
